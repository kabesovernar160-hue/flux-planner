		...
		await context.repositories.food.upsertMany(context.userId, [
			{
				...
				carbs: 0,
				source: 'manual',
				// Названный приём важнее времени сообщения: «на обед борщ»,
				// записанное вечером, — это обед, а не ужин. Если приём
				// не назван, берётся время сообщения.
				meal: intent.meal ?? mealForTime(now, context.timezone)
			} as never
		]);
